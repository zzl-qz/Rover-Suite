...
1. **抄生产样例，不要抄 demo。** 用 `deploy/production/*.example.yml`，改掉所有 token 与 bindHost。
2. **开启严格安全。** `ROVER_STRICT_SECURITY=true`（或 `-Drover.strictSecurity=true`），空 token 直接起不来。
...
9. **日志默认够用。** 访问日志默认 debug + `filters.accessLog`；排障再临时开 DEBUG，别全 INFO 刷盘。
10. **接受内存边界。** Nameserver 进程没了，实例表没了——业务重注册或滚动重启；别按「静默 HA」承诺对外。

做完 1～10 仍卡 404/502/注册，走[故障与性能](./troubleshooting-performance.zh-CN.md)。发布对外前再过一遍[发布检查清单](./release-checklist.zh-CN.md)。
