...
本文只回答两个问题：Gateway 对外暴露哪些可插拔扩展点，以及用户如何亲自验证它们是否生效。
...
这两个扩展点共用一个 `pluginDir`，默认是 Gateway 工作目录下的 `plugins`。所谓“最多 4 条”，只是 2 个扩展点分别支持 SPI 和显式类名/名称两种装配方式，不代表有 4 类插件。插件运行在 Gateway JVM 内，不是独立进程。

注意两者在链路中的角色不同：用户 Filter 插件会作为节点加入过滤器链并按 `getOrder()` 执行；负载均衡策略插件不是 `Filter` 接口实现，而是固定终端环节 `RouteAndProxyFilter` 里的策略插槽，用来从注册中心或静态路由返回的候选实例中选择一个上游。
...
Gateway 还提供一个内置固定阶段 `RateLimitFilter`。它默认关闭，可在 `rover.gateway.rateLimit` 中选择 `token_bucket` 或 `sliding_window`；如果需要按用户、租户或共享存储限流，关闭内置限流并挂载自己的用户 Filter 插件。

对于 Filter，SPI 自动发现适合“JAR 放入后，登记的实现全部加载”，无需在 YAML 重复维护类名；显式类名适合“JAR 可以存在，但由部署配置决定启用哪些类”。后者更灵活，但增加、删除或改名后都要改 `filters.classes` 并重启 Gateway。

对于 LoadBalancer 也同理：SPI `name()` 更适合固定发布的策略名；全限定类名更适合临时验证、灰度切换或不想维护 SPI 文件的场景。无论哪种方式，`gateway.loadbalance.strategy` 都属于启动/插件装配配置，不由 Admin 修改，避免 Admin 覆盖本地 YAML 造成“配置明明写了自定义策略，但运行时被管理台改回内置策略”的问题。
...
编辑 Gateway 的 `rover-gateway.yml`。以下配置展示两个扩展点的两类装配方式，实际使用时按扩展点选择其中一种：
...
日志中同时出现 `Loaded ... filter plugin jar(s)`、`Discovered plugin filter via SPI` 和过滤器链中的插件类，才表示
JAR、SPI 声明与 Filter 实现三者均已对应成功。
...
显式类名方式不需要 SPI 文件；在清除旧构建产物后，保留 JAR 并写入完整类名即可：
...
下图中的“执行自定义Filter”来自 `DemoBlockFilter`，证明请求已进入外部 Filter。

![请求实际进入 demo-block Filter](./images/plugin/filter-plugin-request-executed.png)

无 SPI 的实测示例中，YAML 的 `classes` 指向 `DemoBlockFilter`，请求日志同样出现“执行自定义Filter”。

![无 SPI 的显式类名 Filter 实测](./images/plugin/filter-plugin-class-config-executed.png)

### 3.1 用户自定义限流 Filter 实测

复杂限流不要新增一套 `RateLimiter` 算法 SPI，直接写普通 `Filter` 插件即可。下面这个本地演示把
...
修改 JAR、SPI 文件或类路径后必须重启 Gateway；当前没有持续监听插件目录的 watcher。插件运行在 Gateway 请求链路内，简单判断、打标、内存计数和上游选择通常开销很小；需要避免的是阻塞 I/O、无超时远程调用、无限重试和大对象日志。

## 7. 如果还想继续测用户自定义能力

上面覆盖的是“把外部 JAR 挂进 Gateway”的两个扩展点及其两种装配方式。如果这些都已通过，插件挂载主链路就已经测完。

用户自定义限流也已经走通：它不是新的扩展点，而是普通用户 Filter 插件的一种业务实现。
