...
| 接入其他注册中心 | 源码级 `ServiceDiscovery` 适配与 Gateway 装配 |
...
Filter 和 LoadBalancer 支持 ServiceLoader JAR。ServiceDiscovery 当前只提供源码契约，不会从 `plugins/` 目录加载；不要把三者都宣传成“丢进 JAR 即可”的插件。
...
`getOrder()` 越小越先执行。不要阻塞 Netty 线程，必须返回 `CompletableFuture`；插件实例要线程安全，最好无状态。
...
并调用 `writeText`，因此该类插件会额外耦合 `rover-gateway-core`。对宿主 API 使用 `provided` 依赖，并与宿主版本保持一致。
...
`strategy` 也可以使用实现类全名。`choose` 会被并发调用：不要修改候选列表，内部状态必须线程安全。只有算法需要跟踪在途请求时才实现 `onStart`/`onComplete`。插件实例没有 close 回调。
...
不要为每种语言新增一套 Nameserver endpoint。
...
`NACOS` 和 `REDIS` 枚举值当前是预留项；`rover-gateway-adapter-nacos` 只是骨架，尚不能对外宣称已支持。
...
项目当前是 `1.0.0-SNAPSHOT`，还不应假设二进制兼容已稳定。
