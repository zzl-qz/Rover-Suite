...
服务。默认只发布本机端口，配置中的 token **仅用于演示**，生产环境需要替换。
...
| CORS | 便于本机 Admin 调试 | 默认关 |
| HTTP Registration API | 演示开启 | 无需要则关 |
| 端口映射 | 本机调试用 | 管理口 / Admin **避免**映射公网 |
| 严格安全 | 未开 | `ROVER_STRICT_SECURITY=true` |

**避免把本示例直接用于公网。** 生产请复制 [`../production/`](../production/) 样例，并阅读
...
# 保留运行环境便于继续排查：BENCH_KEEP=1 ./deploy/scripts/bench-small-team.sh
```
...
脚本会打 **两路对照**：经 Gateway（8080）与 **直连 demo**（8082，不经 Gateway/NS），同样使用 `hey -z 30s -c 50`。建议至少重复 3 轮，再记录均值和波动范围。

**完整环境说明、三轮明细与结果解读**见 [`docs-public/performance-report.zh-CN.md`](../../docs-public/performance-report.zh-CN.md)。这组数据是本地 Docker Desktop 参考样本，不代表生产 SLA。
